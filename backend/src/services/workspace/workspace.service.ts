...
export class WorkspaceService {
  ...
  async createProjectWorkspace(metadata: ProjectMetadata): Promise<string> {
  ...
  getSourcePath(projectId: string): string {
    return path.join(this.getProjectPath(projectId), 'src');
  }

  async writeSourceFile(projectId: string, relativePath: string, content: string): Promise<void> {
  ...
  async readSourceFile(projectId: string, relativePath: string): Promise<string> {
    const sourceRoot = this.getSourcePath(projectId);
    const fullPath = path.join(sourceRoot, relativePath);
  ...
  async listProjectFiles(projectId: string, currentDir: string = ''): Promise<string[]> {
    const sourceRoot = this.getSourcePath(projectId);
